...
def register():
    ...
    return flask.render_template("users/register.html", title='Register', form = form)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flask.flash('Login unsuccessful, Please check your email and password!','danger')

    return flask.render_template("users/login.html", title='Login', form = form)
...
def account():
    ...
    image_file = flask.url_for('static', filename = os.path.join('profile_pics',
...
def user_posts(username):
    ...
    return flask.render_template("posts/user_posts.html", title = 'user_posts', posts = posts, user = user)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        return flask.redirect(flask.url_for('users.login'))
    return flask.render_template('users/reset_request.html', title='Reset Password', form = form)
